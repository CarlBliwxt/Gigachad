...
class Lane:
    ...
    def __str__(self):
        # text = "".join(['.' if x is None else x.destination for x in self.lane])
        # return "["+text+"]"
        return '[' + ''.join(['.' if x is None else x.destination    x in self.lane]) + ']'
        # return f'{["." if x is None else x.destination for x in self.lane]}'
